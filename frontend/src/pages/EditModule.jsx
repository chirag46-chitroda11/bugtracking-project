  ...
  useEffect(() => {
    const fetchModule = async () => {
      try {
        ...
        if (module) {
          setForm({ moduleName: module.moduleName, description: module.description });
        }
      } catch (error) { toast.error("Failed to load module"); }
    };
    fetchModule();
  }, [id]);
    ...
    <div className="layout-wrapper relative min-h-screen flex items-center justify-center p-6 text-slate-800">
      ...
      <div className="glass-card">
        <div className="flex items-center gap-4 mb-8">
          ...
          <div>
            <h2 className="text-2xl font-black text-indigo-950 tracking-tight">Edit Module</h2>
            <p className="text-slate-500 font-semibold text-sm">Update specific component logic.</p>
          </div>
        </div>
        ...
        <form onSubmit={handleSubmit}>
          ...
          <textarea className="custom-input h-28 resize-none" placeholder="Description..." value={form.description} onChange={(e) => setForm({ ...form, description: e.target.value })} />
